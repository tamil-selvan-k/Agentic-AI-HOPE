"""The whole project in one command, for the classroom.
    ...
    python -m scripts.demo --crash    # kill the run right after the reservation, resume, count
...
QUESTIONS = [
    ("22CS045", "Is Designing Data-Intensive Applications available? If it is, reserve it for me and send me a text."),
    ("22IT017", "Can I reserve Clean Code?"),
]
...
def counts(db) -> str:
    return (f"reservations {db.count('reservation')}   notifications {db.count('notification')}"
...
def main() -> None:
    ...
    tmp = tempfile.mkdtemp(prefix="library-demo-")
    os.environ["AGENT_DB"], os.environ["LIBRARY_DB"] = os.path.join(tmp, "agent.db"), os.path.join(tmp, "library.db")
        ...
        if a.crash:
            ...
            def once_then_die(key, tool_name, effect):
                ...
                if tool_name == "reserve_book":
                    raise Crash()        # the reservation and its key are committed; nothing after is
                return result
            ...
            try:
                ...
                db.once = real_once
                print(f"\n  {RED}worker-A died right after writing the reservation{RESET}")
    ...
    if a.crash:
        ok = db.count("reservation") == 2 and db.count("notification") == 1      # seed has 1 reservation
        print(f"{GREEN}PASS: one new reservation, one text{RESET}" if ok else f"{RED}FAIL: duplicates{RESET}")
